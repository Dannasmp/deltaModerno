...
export default function CameraScreen({ navigation }) {
  ...
  const takePicture = async () => {
    ...
    try {
      ...
      const logs = guardado ? JSON.parse(guardado) : [];
      logs.unshift(nuevoLog); // La más reciente primero
      await AsyncStorage.setItem(USER_KEY, JSON.stringify(logs));

      Alert.alert(
        "¡Foto guardada!",
        "La imagen fue guardada en el historial.",
